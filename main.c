...
//Guardamos la situaicion original de Mac con port
struct fwd_dst_info dst_info[RTE_MAX_ETHPORTS];

//Guardamos la situacion actual de Mac con port original o alternativo segun su situacion
struct fwd_dst_info dst_table[RTE_MAX_ETHPORTS]; 

//Almacenamiento temporal del puerto destino
static unsigned lsi_temp_port[RTE_MAX_ETHPORTS] = {0} ;

// Indica con 0 con 1 si el puerto esta usando su puerto original o el alternativo
...
#define DEFAULT_TIMER_PERIOD 5UL /* default period is 10 seconds */
...
print_stats(void)
{
	...
	for (portid = 0; portid < RTE_MAX_ETHPORTS; portid++) {
		...
		rte_eth_dev_get_mtu(portid,&mymtu);
		...
		printf("\nStatistics for port %u ------------------------------"
			   ...
			   port_statistics[portid].dropped,mymtu);
...
lsi_simple_forward(struct rte_mbuf *m, unsigned portid)
{
	...
	if(rte_is_broadcast_ether_addr(&eth->dst_addr)){
		rte_ether_addr_copy(&dst_info[0].mac, &eth->dst_addr);
	}

	//Ponemos el puerto destino segun la mac destino
	nb_ports = rte_eth_dev_count_avail();
	for(int i = 0;i<nb_ports;i++){
		if(rte_is_same_ether_addr(&eth->dst_addr,&dst_table[i].mac)){
			dst_port = dst_table[i].port;
			rte_ether_addr_copy(&dst_info[dst_port].mac, &eth->dst_addr);
		}
	}

	rte_ether_addr_copy(&dst_info[2].mac, &eth->src_addr);

	//Añadimos el pquete al buffer
		    ...
		    void *ret_param)
{
	...
	/*ADDED BY SERGI:
	"This function retrieve the link status (up/down), the duplex mode (half/full),
	...
	*/
	...
	//---------------------------------------------------------------------------------------------
	/* EDIT1 SERGI ADDED: Aqui supondremos el caso de que si el puerto destino esta down,
	el paquete se enviara de nuevo por el puerto origen*/
	...
	if(link.link_status){
		...
		for(int portid = 0; portid < nb_ports; portid++) {
			...
			if(lsi_changed_port[portid]==1 && lsi_temp_port[portid]==port_id){
				//printf("\n\nDestionation of %d changed from %d to %d \n\n",portid,lsi_dst_ports[portid],port_id);
				dst_table[portid].port=lsi_temp_port[portid];
				lsi_changed_port[portid] = 0;
				lsi_temp_port[portid] = 0;

				
			}
			printf("\n Sergi:" RTE_ETHER_ADDR_PRT_FMT "; Status %d ; Dest: %d ; TempPort %d",RTE_ETHER_ADDR_BYTES(&dst_table[portid].mac),lsi_changed_port[portid],dst_table[portid].port,lsi_temp_port[portid]);
		}
		
	}else{
		//"Link down"
		printf("\n Sergi: Port DOWN: %d \n",port_id);
		for (int id = 0; id < nb_ports; id++){

			if(dst_table[id].port == port_id && lsi_changed_port[id] == 0){

				lsi_temp_port[id] = dst_table[id].port;
				lsi_changed_port[id] = 1;
				if(lsi_temp_port[id] == nb_ports - 1){
					dst_table[id].port = 0;
				}else{
					dst_table[id].port = lsi_temp_port[id]+1;
				}
				

			}

			printf("\n YO:" RTE_ETHER_ADDR_PRT_FMT "; Status %d ; Dest: %d ; TempPort %d",RTE_ETHER_ADDR_BYTES(&dst_table[id].mac),lsi_changed_port[id],dst_table[id].port,lsi_temp_port[id]);
		}
	}
...
/*SERGI: Parece ser que se realiza una comprobacion del estado
cada 100 ms, por lo que para tener un conteo maximo de las veces
que s ecomprueba, se pone el max check en 90, para 9s maximo */
...
#define MAX_CHECK_TIME 90 /* 9s (90 * 100ms) in total */
	...
	for (count = 0; count <= MAX_CHECK_TIME; count++) {
		...
		if (print_flag == 1){
			...
			//Actualizacion inicial de destinos en estado down
			printf("\nYO: Actualizando Destinos...\n");
			for (portid = 0; portid < port_num; portid++) {
				memset(&link, 0, sizeof(link));
				ret = rte_eth_link_get_nowait(portid, &link);
				...
				if(link.link_status){
					...
					//"Link down"
					for (int id = 0; id < RTE_MAX_ETHPORTS; id++){
						if(dst_table[id].port == portid && lsi_changed_port[id] == 0){
						lsi_temp_port[id] = dst_table[id].port;
						lsi_changed_port[id] = 1;
						dst_table[id].port = lsi_temp_port[id]+1;
						}
			...
			for(int portid3 = 0; portid3 < port_num; portid3++){
				printf("\n YO:" RTE_ETHER_ADDR_PRT_FMT "; Status %d ; Dest: %d ; TempPort %d",RTE_ETHER_ADDR_BYTES(&dst_table[portid3].mac),lsi_changed_port[portid3],dst_table[portid3].port,lsi_temp_port[portid3]);
			}
...
//Carga las direcciones mac destino del archivo macs.txt 
// + los guarda en las estructuras dst_info[i].port y dst_table[i].port
int get_dst_mac(uint16_t portnum){

    char mac_str[18];
    int i=0;
    FILE *fp = fopen("/home/i2t-sergi/dpdk/dpdk-23.11/examples/mylinkv3_status_interrupt/macs.txt","r");
    ...
    while(fgets(mac_str, sizeof(mac_str),fp) != NULL || i<portnum){
		...
		dst_info[i].port=i;
		dst_table[i].port=i;
        printf("Dest Port %d , MAC address: " RTE_ETHER_ADDR_PRT_FMT "\n",dst_info[i].port,RTE_ETHER_ADDR_BYTES(&dst_info[i].mac));
        i++;
    }
...
int rte_ether_unformat_addr(const char *str, struct rte_ether_addr *eth_addr)
{
	...
	if(n == 6) {
		...
		eth_addr->addr_bytes[0] = o0;
		eth_addr->addr_bytes[1] = o1;
		eth_addr->addr_bytes[2] = o2;
		eth_addr->addr_bytes[3] = o3;
		eth_addr->addr_bytes[4] = o4;
		eth_addr->addr_bytes[5] = o5;
...
main(int argc, char **argv)
{
	...
	////Edit1 Added by sergi ------------------------------------------------------------------------
	printf("\n----- YO: INICIALIZANDO DESTINOS Y BACKUPS -----\n");
	//------------------------------------------------------------------------
	for (portid = 0; portid < nb_ports; portid++) {
		...
		if(portid==(nb_ports - 1)){
			lsi_dst_ports[portid] = 0;
		}else{
			lsi_dst_ports[portid] = portid+1;
		}
	...
	for (portid = 0; portid < nb_ports; portid++) {
		...
		printf("\nCallback registrado para el puerto %d \n",portid);
